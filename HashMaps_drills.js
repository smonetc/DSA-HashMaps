...
function main() {
  ...
  console.log('LOTR', lotr)//length is 9, collisions under Hobbit and Maiar
  ...
  console.log(lotr._capacity); // 24, we exceed the initial length of 8 we must multiply by size ratio to accomodate - 8 x 3. 
}

main()

//will overwrite previous data values on any duplicate keys the function finds - believe this would be a collision due to the overwrite.
...
console.log(removeDuplicates("how i want to go out to the mall"));
...
//   // otherwise, get the character and attribute a value of "1".
  for (let i = 0; i < string.length; i++) {
      ...
      try {
          let value =  palindromeMap.get(character);
          value++;
          palindromeMap.set(character,value);
      }
      catch(error) {
        palindromeMap.set(character, 1);
      }
  }

//   // loop through to check for palindromes, return true/false based on condition.
  for (let i = 0; i < string.length; i++) {
      let checker =  palindromeMap.get(string[i]);
...
function separateChain() {
  ...
  console.log("LOTR", lotr); // structure is displaying correct, but there is still a collision happening
}
